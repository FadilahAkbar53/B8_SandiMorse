...
// Fungsi untuk mengenkripsi pesan menjadi kode Morse
...
// Fungsi untuk mendekripsi kode Morse menjadi pesan
void fitur_decodeMorse(TreeNode *root) {
    ...
    while (*input) {
        if (*input == ' ') {
            ...
            char decodedChar = decode(root, buffer);
            insertLast(&decodedList, (decodedChar != '\0') ? decodedChar : '?');
            bufferIndex = 0;
        } else {
    ...
    char *result = linkedListToString(decodedList);
    printf("\nKarakter:\n%s\n", result);

    free(result);
    freeLinkedList(&decodedList);
    free(input);
}

// Fungsi untuk mencetak pohon Morse secara inorder
...
void findPath(TreeNode *root, char start, char end) {
    ...
    if (startNode == NULL) {
        printf("Karakter awal '%c' tidak ditemukan dalam pohon Morse.\n", start);
        return;
    }

    if (endNode == NULL) {
        printf("Karakter akhir '%c' tidak ditemukan dalam pohon Morse.\n", end);
        return;
    }

    char pathStart[100], pathEnd[100];
    int PathDirecStart[100], PathDirecEnd[100];
    int pathStartLen = findPathToRoot(startNode, pathStart, PathDirecStart);
    int pathEndLen = findPathToRoot(endNode, pathEnd, PathDirecEnd);
    ...
    printf("Jalur dari '%c' ke '%c': ", start, end);
    for (int k = 0; k <= i; k++) {
        if (PathDirecStart[k] == 0 || k == 0) { // Cetak hanya jika itu adalah anak kiri atau karakter pertama
