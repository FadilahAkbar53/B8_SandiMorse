#ifndef B8_SANDIMORSE_UTIL_H
#define B8_SANDIMORSE_UTIL_H
...
void reverseArray(char arr[], int n) {
    ...
    while (start < end) {
        // Tukar nilai di indeks 'start' dengan nilai di indeks 'end'
        int temp = arr[start];
        arr[start] = arr[end];
        arr[end] = temp;

        // Pindahkan indeks 'start' ke depan dan indeks 'end' ke belakang
        start++;
        end--;
    }
}
